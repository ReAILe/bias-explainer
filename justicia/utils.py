...
def get_population_model_verifair_format(data, sensitive_attribute):
    ...
    if(divide):
        s += "\tif " + str(sensitive_attribute) + " < 1:\n"
        for column in data.columns:
            if(column == sensitive_attribute):
                continue
            if(len(data[column].unique()) == 2):
...
def get_population_model_fairsquare_boolean_format(data, sensitive_attribute, distinct_nodes):

    # find sensitive attribute from distinct_nodes
    proxy_sensitive_attribute = sensitive_attribute
    found_indices = []
    for i, (node, threshold) in enumerate(distinct_nodes.keys()):
        if(node == sensitive_attribute):
            found_indices.append(i)
    assert len(found_indices) < 2 # at most one
    if(len(found_indices) == 1):
        proxy_sensitive_attribute = 'x_' + str(found_indices[0])
    ...
    mean = data[sensitive_attribute].mean()
    mean = round(mean, 7)
    s += "\t" + proxy_sensitive_attribute + \
        " = step([(0,0.5," + str(round(1-mean, 7)) + \
        "), (0.5,1," + str(mean) + ")])\n"

    if(divide):
        s += "\tif " + str(proxy_sensitive_attribute) + " < 0.5:\n"
        # similar to get_population_model_verifair_boolean_format
        for (node, threshold) in distinct_nodes:
            if(node == sensitive_attribute):
                continue
            
            mean = data[(data[sensitive_attribute] < 0.5) & (data[node] <= threshold)][node].mean()
            mean = round(mean, 7)
            s += "\t\t" + "x_" + str(distinct_nodes[(node, threshold)]) + \
                " = step([(0,0.5," + str(round(1 - mean, 7)) + \
                "), (0.5,1," + str(mean) + ")])\n"
        ...
        for column in data.columns:
            ...
            if(len(data[column].unique()) == 2):
                ...
                s += "\t" + column + \
                    " = gaussian(" + str(mu) + "," + str(std) + ")\n"
    s += "\tsensitiveAttribute(" + proxy_sensitive_attribute + " < 0.5)\n\n\n\n"
    return s


def get_population_model_verifair_boolean_format(data, sensitive_attribute, distinct_nodes):
    ...
    if(divide):
        s += "\tif " + str(proxy_sensitive_attribute) + " < 1:\n"
        for (node, threshold) in distinct_nodes:
            if(node == sensitive_attribute):
                continue
        ...
        for column in data.columns:
            ...
            if(len(data[column].unique()) == 2):
                ...
                s += "\t" + column + \
                    " = gaussian(" + str(mu) + "," + str(std) + ")\n"
    s += "\tsensitiveAttribute(" + proxy_sensitive_attribute + " < 1, flag)\n\n\n\n"
    return s
...
def tree_to_code(tree, dataset_name, feature_names, method='fairSquare'):
    ...
    s = None
    if(method == 'fairSquare'):
        s = "def tree({}):".format(", ".join(feature_names)) + "\n\n"
        s = "def F():\n"
    elif(method == 'veriFair'):
        s = "def tree({}):".format(", ".join(feature_names)) + "\n\n"
        s = ""
    else:
    ...
    s = recurse(0, 1, s)
    if(dataset_name in["titanic", "adult", "german"]):
        if(method == 'fairSquare'):
...
def tree_to_code_boolean_format(tree, dataset_name, feature_names, method='fairSquare'):
    ...
    def recurse(node, depth, s):
        ...
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            threshold = round(tree_.threshold[node], 7)
            index = None
            if((name, threshold) not in distinct_nodes):
                distinct_nodes[(name, threshold)] = node_iterator
                index = node_iterator
                node_iterator += 1
            else:
                # print("Yes", (name, threshold), distinct_nodes[(name, threshold)])
                index = distinct_nodes[(name, threshold)]
            # s = s + "{}if {} <= {}:".format(indent, name, threshold) + "\n"
            if(method == 'fairSquare'):
                s = s + "{}if {} <= {}:".format(indent, 'x_' + str(index), 0.5) + "\n"
            elif(method == 'veriFair'):
                s = s + "{}if {} <= {}:".format(indent, 'x_' + str(index), 1) + "\n"
            else:
                raise ValueError("Method not supported")
            s = recurse(tree_.children_left[node], depth + 1, s)
            s = s + "{}else:".format(indent) + "\n"
